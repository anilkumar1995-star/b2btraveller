    ...
    public function export(Request $post, $type)
    {
        ...
        switch ($type) {
            ...
            case 'billpay':
                ...
                $query->leftJoin('billpay_providers', 'billpay_providers.id', '=', 'reports.provider_id');
            ...
            case 'money':
                $table = "aepsreports.";
                $query = \DB::table('aepsreports')->where($table . 'rtype', 'main')->where($table . 'product', "payout");
                ...
                $query->leftJoin('billpay_providers', 'billpay_providers.id', '=', 'aepsreports.provider_id');
                $query->leftJoin('users as emloyee', 'emloyee.id', '=', 'distributor.parent_id');
                if (isset($post->agent) && $post->agent != '' && $post->agent != 0) {
                    $query->where($table . 'user_id', $post->agent);
                } else {
            ...
            case 'matm':
                ...
                $query->leftJoin('billpay_providers', 'billpay_providers.id', '=', 'microatmreports.provider_id');
            ...
            case 'other':
                ...
                if ($type != 'kycpending') {
                    ...
                    $query->whereIn('users.kyc', ['pending']);
                }
                $query->whereIn($table . 'id', $parentData);
                break;
            ...
            case 'fund':
                ...
                $query->leftJoin('users as retailer', 'retailer.id', '=', 'reports.user_id');
                $query->leftJoin('users as sender', 'sender.id', '=', 'reports.credited_by');
        ...
        if (
            (isset($post->fromdate) && !empty($post->fromdate))
            && (isset($post->todate) && !empty($post->todate))
        ) {
        ...
        switch ($type) {
            ...
            case 'money':
                $datas = $query->get(['aepsreports.id', 'aepsreports.created_at', 'aepsreports.number', 'aepsreports.amount', 'aepsreports.charge', 'aepsreports.profit', 'aepsreports.txnid', 'aepsreports.refno', 'aepsreports.mobile', 'aepsreports.status', 'aepsreports.wid', 'aepsreports.wprofit', 'aepsreports.mdid', 'aepsreports.mdprofit', 'aepsreports.disid', 'aepsreports.disprofit', 'retailer.id as userid', 'retailer.name as username', 'retailer.mobile as usermobile', 'retailer.state as userstate', 'retailer.pincode as userpincode', 'whitelable.name as wname', 'whitelable.mobile as wmobile', 'md.mobile as mdmobile', 'distributor.name as disname', 'distributor.mobile as dismobile', 'apis.name as apiname', 'billpay_providers.name as providername', 'aepsreports.user_id', 'emloyee.id as empid', 'emloyee.name as empname', 'emloyee.mobile as empmobile']);
                break;

            case 'aeps':
                $datas = $query->get(['aepsreports.id', 'aepsreports.created_at', 'aepsreports.mobile', 'aepsreports.aadhar', 'aepsreports.amount', 'aepsreports.refno', 'aepsreports.charge', 'aepsreports.status', 'aepsreports.aepstype', 'aepsreports.wid', 'aepsreports.wprofit', 'aepsreports.mdid', 'aepsreports.mdprofit', 'aepsreports.disid', 'aepsreports.disprofit', 'retailer.name as username', 'retailer.mobile as usermobile', 'whitelable.name as wname', 'whitelable.mobile as wmobile', 'md.name as mdname', 'md.mobile as mdmobile', 'distributor.name as disname', 'distributor.mobile as dismobile', 'distributor.role_id as drole', 'apis.name as apiname', 'billpay_providers.name as providername', 'aepsreports.user_id', 'retailer.id as userid', 'retailer.name as username', 'retailer.mobile as usermobile', 'retailer.state as userstate', 'retailer.pincode as userpincode', 'emloyee.id as empid', 'emloyee.name as empname', 'emloyee.mobile as empmobile']);
                break;

            case 'matm':
                $datas = $query->get(['microatmreports.id', 'microatmreports.created_at', 'microatmreports.mobile', 'microatmreports.aadhar', 'microatmreports.amount', 'microatmreports.refno', 'microatmreports.charge', 'microatmreports.status', 'microatmreports.wid', 'microatmreports.wprofit', 'microatmreports.mdid', 'microatmreports.mdprofit', 'microatmreports.disid', 'microatmreports.disprofit', 'retailer.name as username', 'retailer.mobile as usermobile', 'whitelable.name as wname', 'whitelable.mobile as wmobile', 'md.name as mdname', 'md.mobile as mdmobile', 'distributor.name as disname', 'distributor.mobile as dismobile', 'apis.name as apiname', 'billpay_providers.name as providername', 'microatmreports.user_id']);
                break;
            ...
            case 'fund':
                $datas = $query->get(['reports.id', 'reports.created_at', 'reports.amount', 'reports.refno', 'reports.product', 'reports.status', 'retailer.name as username', 'retailer.mobile as usermobile', 'sender.name as sendername', 'sender.mobile as sendermobile', 'reports.user_id', 'reports.credited_by', 'reports.remark']);
                break;
            ...
            case 'aepsagentstatement':
                $datas = $query->get(['agents.id', 'agents.created_at', 'agents.bc_id', 'agents.bc_f_name', 'agents.bc_l_name', 'agents.bc_l_name', 'agents.emailid', 'agents.phone1', 'agents.phone2']);
                break;
            ...
            case 'verification':
            case 'awallet':
                $datas = $query->get(['aepsreports.id', 'aepsreports.created_at', 'aepsreports.payid', 'aepsreports.remark', 'aepsreports.aadhar', 'aepsreports.mobile', 'aepsreports.refno', 'retailer.name as username', 'retailer.mobile as usermobile', 'aepsreports.user_id', 'aepsreports.trans_type', 'aepsreports.rtype', 'aepsreports.status', 'aepsreports.balance', 'aepsreports.amount', 'aepsreports.charge', 'aepsreports.type']);
                break;
        ...
        switch ($type) {
            ...
            case 'pancard':
                $name = $type . 'report' . date('d_M_Y');
                $titles = ['Transaction Id', 'Created Date', 'Api Name', 'Number', 'Option1', ' Amount', 'Charge', 'Profit', 'Ref No', 'Status', "Member Details"];
            ...
            case 'money':
                $name = $type . 'report' . date('d_M_Y');
                $titles = ['Id', 'Created At', 'Number', 'Amount ', 'Charge', 'Profit', 'Transaction Id', 'Reference No', 'Mobile', 'Status', 'User id', 'Retailer Username', 'Retailer Mobile', 'Retailer State', 'Retailer Pincode', 'API name', 'Provider name', 'User Details', 'Emp Details'];

                if (\Myhelper::hasRole(['distributor', 'md', 'whitelable', 'admin'])) {
                    $titles = array_merge($titles, ["Disid", "Distributor Details", "Distributor Profit"]);
                }

                if (\Myhelper::hasRole(['md', 'whitelable', 'admin'])) {
                    $titles = array_merge($titles, ["Mdid", "Md Details", "Md Profit"]);
                }

                if (\Myhelper::hasRole(['whitelable', 'admin'])) {
                    $titles = array_merge($titles, ["Wid", "Whitelable Details", "Whitelable Profit"]);
                }
                ...
                foreach ($datas as $record) {
                    ...
                    $data['userstate'] = $record->userstate;
                    $data['userpincode'] = $record->userpincode;
                    $data['apiname'] = $record->apiname;
                    $data['providername'] = $record->providername;
                    $data['userdetails'] = $record->username . " (" . $record->usermobile . ")";
                    $data['empid'] = $record->empid . " (" . $record->empname . ")";

                    if (\Myhelper::hasRole(['distributor', 'md', 'whitelable', 'admin'])) {
                        $data['disid'] = $record->disid;
                        $data['disdetails'] = $record->disname . " (" . $record->dismobile . ")";
                        $data['disprofit'] = $record->disprofit;
                    }

                    if (\Myhelper::hasRole(['md', 'whitelable', 'admin'])) {
                        $data['mdid'] = $record->mdid;
                        // $data['mddetails'] = $record->mdname . " (" . $record->mdmobile . ")";
                        $data['mdmobile'] = $record->mdmobile;
                        $data['mdprofit'] = $record->mdprofit;
                    }
            ...
            case 'employee':
                $name = $type . 'report' . date('d_M_Y');
                $titles = ['Id', 'Date', 'Name', 'Email', 'Mobile', 'Role Name', 'Main Balance', 'Aeps Balance', 'Parent', 'Company', 'Status', 'address', 'City', 'State', 'Pincode', 'Shopname', 'Gst Tin', 'Pancard', 'Aadhar Card', 'Account', 'Bank', 'Ifsc'];
                foreach ($datas as $record) {
        ...
        // $file_path = public_path("reports/report.csv");
        ...
        if (file_exists(public_path() . 'reports/report.csv')) {
            unlink(public_path() . 'reports/report.csv');
        }
        $path = public_path() . "/reports";
        $filename = 'report.csv';
        //  $csvReport = $path.'/'.$filename;
