...
class TransactionController extends Controller
{
	public function transaction(Request $request)
	{
		...
		switch ($request->type) {
			...
			case 'aepsStatement':
				...
				$request['select'] = ['txn_id as txnId', 'message', 'mobile_no as mobileNo', 'order_ref_id as orderRefId', 'bankName', 'rrn', 'account_number as accountNumber', 'operator_ref_id as operatorRefId', 'txn_mode as txnMode', "pay_amount as payAmount", "txn_value as txnValue", "account_balance as accountBalance", "txn_date_time as txnDateTime", "txn_type as txnType", "txn_amount as txnAmount", "merchant_code as merchantCode", "status", "created_at as createdAt", "updated_at as updatedAt"];
				$request['order'] = ['id', 'DESC'];
			...
			case 'aepsWalletStatement':
				...
				$request['select'] = "all";
				// ['id','mobile','aadhar','merchant_code as merchantCode','txnid as txnId','api_id as apiId','amount','charge','profit','bank','payid as payId','refno as refNo','mytxnid','terminalid as terminalId','authcode','balance as balance','status','type','remark','rtype','trans_type as transType','aepstype as aepsType','TxnMedium','user_id as userId','credited_by','via','apitxnid','provider_id as providerId','udf1','udf2','udf3','udf4','udf5','udf6','description','wid','wprofit','mdid','mdprofit','disid','disprofit','adminprofit','product','created_at','updated_at','apicode','username'];
				$request['order'] = ['id', 'DESC'];
				$request['parentData'] = [$request->user_id];
				$request['whereIn'] = 'user_id';
				break;
			// $request['table'] = '\App\Models\CommissionTable';
			// $request['searchdata'] = ['mobile', 'aadhar', 'txnid', 'refno', 'payid', 'amount'];
			// $request['select'] = "all";
			// // ['id','mobile','aadhar','merchant_code as merchantCode','txnid as txnId','api_id as apiId','amount','charge','profit','bank','payid as payId','refno as refNo','mytxnid','terminalid as terminalId','authcode','balance as balance','status','type','remark','rtype','trans_type as transType','aepstype as aepsType','TxnMedium','user_id as userId','credited_by','via','apitxnid','provider_id as providerId','udf1','udf2','udf3','udf4','udf5','udf6','description','wid','wprofit','mdid','mdprofit','disid','disprofit','adminprofit','product','created_at','updated_at','apicode','username'];
			// $request['order'] = ['id', 'DESC'];
			...
			case 'payoutStatement':
				...
				break;
			case 'cmsstatement':
			case 'commissionWalletStatement':
			...
			case 'matmfundrequest':
				...
				break;
			default:
				return ResponseHelper::success('Report fetched Successfully', []);
		}
		...
		try {
			...
			foreach ($data as $key => $val) {
				if (isset($val->closing_balance)) {
					unset($val->closing_balance);
				}
			}
		...
		return ResponseHelper::success("Report Fetched Successfully", ['pages' => $totalpage, "txnData" => $data]);
	}
	...
	public function getData($request, $returntype)
	{
		...
		switch ($request->type) {
	    	case 'upipayoutStatement':
		   	$data->where('product', 'upipayout')->where('rtype', 'main'); 
		    break;
			case 'distributor':
			case 'retailer':
			...
			case 'dmtstatement':
				$data->whereIn('product', ['dmt','payout']);
				break;
			case 'mainWalletStatement':
