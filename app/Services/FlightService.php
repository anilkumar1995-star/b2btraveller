...
class FlightService
{
    ...
    {
        if ($method == 'search') {
            return $this->baseUrl . '/v1/service/traveller/flight/search';
        } else if ($method == 'farerule') {
            return $this->baseUrl . '/v1/service/traveller/flight/farerule';
        } else if ($method == 'farequote') {
            return $this->baseUrl . '/v1/service/traveller/flight/farequote';
        } else if ($method == 'ssr') {
            return $this->baseUrl . '/v1/service/traveller/flight/ssr';
        } else if ($method == 'book') {
            ...
            if ($data['JourneyType'] == 2 && isset($data['Segments']) && count($data['Segments']) == 2) {
                ...
                $payload["Segments"] = [
                    [
                        ...
                        "PreferredreTime" => $data['Segments'][0]['PreferredreTime'],
                    ...
                    [
                        ...
                        "FlightCabinClass" => $data['FlightCabinClass'],
                        "PreferredreTime" => $data['Segments'][1]['PreferredreTime'],
                ...
                $payload["Segments"] = [
                    [
                        ...
                        "PreferredreTime" => $data['PreferredreTime'] ?? date('Y-m-d\TH:i:s'),
            ...
            if (isset($response['status']) && $response['status'] == 'SUCCESS') {
                return ['status' => 'success', 'message' => "Flight Booking successfully", 'data' => $response['data']];
                 ...
                 return [
                    'code' => $response['code'] ?? '0x0202',
                    'status' => $response['status'] ?? 'failed',
                    'message' => $response['message'] ?? 'Flight Booking get failed'
                ];
            }
        } catch (Exception $e) {
            return ['status' => 'ERROR', 'message' => $e->getMessage()];
    ...
    public function FlightTicketView($data)
    {
        try {
            ...
            if (isset($response['status']) && $response['status'] == 'SUCCESS') {
                return ['status' => 'success', 'message' => "Flight Booking successfully Done", 'data' => $response['data']];
            } else {
                 return [
                    'code' => $response['code'] ?? '0x0202',
                    'status' => $response['status'] ?? 'failed',
