...
$('#flightSearchForm').on('submit', function (e) {
    ...
    if (totalTraveler > 9) {
        ...
        if (activeContent === 'pills-one-way') {
            ...
            payload.OriginCountry = $('#OriginCountry').val();
            payload.DestinationCountry = $('#DestinationCountry').val();
            ...
            payload.Segments = [
                {
                    ...
                    OriginCountry: $('#RoundOriginCountry').val()
                },
                {
                    ...
                    DestinationCountry: $('#RoundDestinationCountry').val()
                }
            ];
        }
...
function getFareRulesInternationalRoundtrip(resultIndex, traceId) {
    if (!resultIndex || !traceId) return;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            const fareRules = response?.data?.FareRules || [];
            ...
            $('#importantInfoSectionDeparture').html(depHtml || 'No Data Available');
            $('#importantInfoSectionReturn').html(retHtml || 'No Data Available');
        },
        error: function () {
            notify("Failed to fetch fare rule. Please try again.", "error");
        }
...
function getInternationalRoundTripFareQuote(resultIndex, traceId) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 'success') {
                    ...
                    localStorage.setItem(`fareFlightDetails`, JSON.stringify(resultData?.FareBreakdown));
                    // let segmnt = resultData.Segments[0];
                    let seg = resultData?.Segments || [];
                    seg.forEach((segmnt, indx) => {
                        const trip = indx === 0 ? 'departure' : 'return';
                        const fmt = (num) => Number(num || 0).toLocaleString('en-IN');
                    ...
                    `;
                        ...
                        if (resultData?.Segments[0].length != 0) {
                            ...
                            resultData?.Segments[indx].forEach((segs, index) => {
                                bagDetHtml += `
                                    <tr>
                                        <td>Adult 🧳 (Segment ${index + 1} - ${segs.Craft})</td>
                                        <td>${segs.Baggage}</td>
                                        <td>${segs.CabinBaggage}</td>
                                    </tr>
                                `;
                            });
                        ...
                        if (resultData?.IsCouponAppilcable) {
                            ...
                            couponHtml += ` <div class="card card-body bg-light mb-3">
                                 <form>
                                    <div class="card-title">
                                        <h5>Offer & Discount for International Flight (Roundtrip)</h5>
                                    </div>
...
function displayInternationalRTFlightDetails(flightDetails, trip, segmentIndex) {

    let segs = flightDetails?.Segments?.[segmentIndex] || [];
    ...
    titledetailsHtml = `
        <h1 class="display-4 mb-0">
            <i class="fa-solid fa-plane rtl-flip fs-1"></i>
        </h1>
        <div class="ms-3">
            <ul class="list-inline mb-2">
                <li class="list-inline-item me-2">
            ...
            <ul class="nav nav-divider h6 fw-normal text-body mb-0">
                <li class="nav-item">${fmtDate(firstSeg.Origin.DepTime)}</li>
                <li class="nav-item">
                    &nbsp;|&nbsp;${segs.length > 1 ? segs.length - 1 : 'Non'} Stop
                </li>
                <li class="nav-item badge bg-label-warning text-uppercase">
                    ${trip}
                </li>
            </ul>
        </div>
    ...
    for (let i = 0; i < segs.length; i++) {
        ...
        <div class="row g-4">
            ...
            <div class="col-md-3">
                ...
                <h6>${fmtTime(s.Origin.DepTime)}</h6>
                <p>${fmtDate(s.Origin.DepTime)}</p>
                <p>${s.Origin.Airport.AirportName}</p>
                <p>Terminal: ${s.Origin.Airport.Terminal || 'N/A'}</p>
            </div>
            ...
            <div class="col-md-3 text-end">
                ...
                <h6>${fmtTime(s.Destination.ArrTime)}</h6>
                <p>${fmtDate(s.Destination.ArrTime)}</p>
                <p>${s.Destination.Airport.AirportName}</p>
                <p>Terminal: ${s.Destination.Airport.Terminal || 'N/A'}</p>
            </div>
        </div>
        `;
        ...
        if (i < segs.length - 1) {
            ...
            detailsHtml += `
                <div class="bg-light rounded-2 text-center text-danger p-2 mb-4">
                    Ground Time at ${s.Destination.Airport.CityName}:
                    ${groundTime}
                </div>
            `;
        }
            ...
            <div class="accordion-item mb-3 border">
                <h2 class="accordion-header">
                    <button class="accordion-button collapsed"
                        ...
                        <div class="d-flex align-items-center"
                             id="titleSectionReturn"></div>
                    </button>
                </h2>
                <div id="collapseReturn"
                     class="accordion-collapse collapse">
                    <div class="accordion-body mt-3"
                         id="getSelectFlightDetailsReturn"></div>
                </div>
            </div>
        `);
...
function isInternationalFlight(results) {
    if (!results.length) return false;

    const firstSegment = results[0][0]?.Segments?.[0]?.[0];
    if (!firstSegment) return false;

    const originCountry = firstSegment.Origin.Airport.CountryCode;
...
function roundtripFlightResults(data) {
    ...
    if (!results.length) {
        notify("No flights available", "error");
        $('.all_flight_list').addClass('d-none');
        return;
    }

    isInternational = isInternationalFlight(results);

    if (isInternational) {

        const segments = results[0]?.[0]?.Segments || [];

        if (segments.length === 2) {
            finalResults = results

            $('#tabReturnItem').addClass('d-none');
            $("#tabDeparture").addClass("active");
        } else {
            notify("Invalid return response for international round trip", "error");
            $('.all_flight_list').addClass('d-none');
            $('#tabReturnItem').removeClass('d-none');
            return;
        }
        $("#roundTabs").addClass("d-none");
    } else {
        ...
        $('#tabReturnItem').removeClass('d-none');
        if (results.length < 2) {
            notify("Round Trip data incomplete. Only onward flight received", "error");
            $('.all_flight_list').addClass('d-none');
            return;
        }
        finalResults = results;
...
function renderRoundList(index) {
    ...
    groups.forEach((flight, j) => {
        if (isInternational) {
            renderInternationalRound(groups, index);
        } else {
            let segs = flight.Segments[0];
            let hasLayover = segs.length > 1;
            let layovers = segs.length - 1;
...
function renderInternationalRound(groups, index) {
    ...
    groups.forEach((flight) => {
        ...
        if (flight.Segments[0] && flight.Segments[0][0]) {
            parentHtml += renderSingleCard(flight, flight.Segments[0], index, "Onward");
        }
        // ================= RETURN CARD =================
        if (flight.Segments[1] && flight.Segments[1][0]) {
            parentHtml += renderSingleCard(flight, flight.Segments[1], index, "Return");
        }
        ...
        parentHtml += `
            ...
            <div class="card-footer bg-white d-flex justify-content-between align-items-center p-3">
                <h4 class="mb-0">₹${flight.Fare.PublishedFare}</h4>
                <button class="btn btn-primary btn-book-now" data-bookingflightdetails='${encodeURIComponent(JSON.stringify(flight))}'>
                    Book Now
                </button>
            </div>
        </div>
        ...
        $("#search_flight_list").append(parentHtml);

    });



    function renderSingleCard(flight, seg, index, tripType) {
        const firstSeg = seg[0];
        const lastSeg = seg[seg.length - 1];

        let travelClass = cabinMap[seg.CabinClass] || 'Economy';
        let seats = firstSeg.NoOfSeatAvailable || 0;
        let lcc = flight.IsLCC ? 'LCC' : 'Non-LCC';
        let layovers = seg.length - 1;
            ...
            <div class="card-header d-flex justify-content-between">
                <div class="d-flex align-items-center">
                    ...
                    <h6 class="fw-normal mb-0">
                        ...
                        <span class="badge bg-secondary ms-2">${tripType}</span>
                    </h6>
                </div>
                <h6 class="fw-normal mb-0">${travelClass} (${lcc})</h6>
            </div>
            ...
            <div class="card-body p-4">
                <div class="row g-4 align-items-center">
                    <div class="col-md-12">
                        <div class="row g-4">
                            <div class="col-sm-4">
                                <h4>${fmtTime(firstSeg.Origin.DepTime)}</h4>
                                <h6 class="fw-normal mb-0">${fmtDate(firstSeg.Origin.DepTime)}</h6>
                                <p>${firstSeg.Origin.Airport.CityName}</p>
                            </div>

                            <div class="col-sm-4 text-center">
                                <h5>${formatDuration(firstSeg.Duration)}</h5>
                                <div class="position-relative my-4">
                            ...
                            <div class="col-sm-4 text-end">
                                <h4>${fmtTime(firstSeg.Destination.ArrTime)}</h4>
                                <h6 class="fw-normal mb-0">${fmtDate(firstSeg.Destination.ArrTime)}</h6>
                                <p>${firstSeg.Destination.Airport.CityName}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-footer">
                ...
                : `<span class="text-success fw-bold">Non-stop</span>`
                    ...
                    <li class="list-inline-item">
                        <button class="btn p-0 text-primary view-details" 
                                data-bs-toggle="modal" data-bs-target="#flightdetail" 
                            data-segs='${JSON.stringify(seg)}' data-allDetails='${JSON.stringify(flight)}'>
                            👁️ View Details <i class="fa-solid fa-angle-right ms-1"></i>
                        </button>
                    </li>
                </ul>
