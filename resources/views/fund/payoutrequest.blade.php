...
<div class="offcanvas offcanvas-end" id="transferModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static">
            <div class="offcanvas-header bg-primary">
                <h4 class="text-white" id="exampleModalLabel">Fund Request From</h4>
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    if (full.status == "approved") {
                        var btn = `<span class="badge badge-success bg-success">${full?.status}</span>`;
                    } else if (full.status == 'pending') {
                        var btn = `<span class="badge badge-warning bg-warning">${full?.status}</span>`;
                    } else {
                        var btn = `<span class="badge badge-danger bg-danger">${full?.status}</span>`;
                    }
                    @if(Myhelper::hasRole('admin'))
