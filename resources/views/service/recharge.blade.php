...
<style>
    ...
    <iv class="content">
                ...
                <form id="rechargeForm" action="{{ route('rechargepay') }}" method="post" autocomplete="off">
                    ...
                    <div class="row">
                        <div class="form-group col-sm-12 col-lg-4 my-1">
                            ...
                            <input type="text" name="number" class="form-control my-1"
                                placeholder="Enter {{ $type }} number" required="">
                            <!--onchange="getoperator()"-->
                        </div>
                        <div class="form-group col-sm-12 col-lg-4 my-1">
                            <label class="mb-1">{{ ucfirst($type) }} Operator <span class="text-danger fw-bold">*</span></label>
                            <select name="provider_id" class="form-control my-1 select" required="" onchange="getdthinfo()">
                        ...
                        @if ($type == 'mobile')
                            <div class="form-group col-sm-12 col-lg-4 my-1">
                                <label class="mb-1">Circle</label>
                                <select name="circle" class="form-control my-1 select" id="circle" required>
                            ...
                            <div class="form-group col-sm-12 col-lg-4 my-1">
                                <label class="mb-1">Plan Type</label>
                                <select name="rechargeType" class="form-control my-1 select" id="rechargeType" required>
                        ...
                        <div class="form-group col-sm-12 col-lg-4 my-1">
                            ...
                            <div class="input-group my-1"><input type="text"name="amount" autocomplete="off"
                                    class="form-control" placeholder="Enter amount"required>
                                <span class="input-group-text bg-transparent border-start-0">
                                    <a href="javascript:void(0)" onclick="getplan()"
                                        class="text-success fw-semibold text-decoration-none"><i
                                            class="fas fa-clipboard-list me-1"></i> Get Plan</a>
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="form-group col-sm-12 col-lg-4 my-1">
                            ...
                            <input type="password" name="pin" class="form-control my-1" autocomplete="off"
                                placeholder="Enter transaction pin" required="">
                            <a href="{{ url('profile/view?tab=pinChange') }}" target="_blank"
                                class="text-primary pull-right">Generate Or Forgot Pin??</a>
                        </div>

                        <div class="col-sm-12 col-lg-12 mt-4 text-end">
        ...
        <div class="modal fade bd-example-modal-xl" id="planModal" tabindex="-1" role="dialog"
        ...
        @if ($type == 'dth')
            <div class="modal fade" id="dthinfo" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                    ...
                    submitHandler: function() {
                        ...
                        form.ajaxSubmit({
                            ...
                            beforeSubmit: function() {
                                form.find('button[type="submit"]').html('Processing...').attr(
                                    'disabled', true).addClass('btn-secondary');
                            },
                            complete: function() {
                                form.find('button[type="submit"]').html('Pay Now').attr(
                                    'disabled', false).removeClass('btn-secondary');
                            },
                            success: function(data) {
                                if (data.status == "success" || data.status == "pending" || data
                                    .statuscode == "TXN" || data.statuscode == "TXF" || data
                                    .statuscode == "TUP") {
                                    getbalance();
                                    form[0].reset();
                                    form.find('select').val(null).trigger('change')
                                    if (data.status == "success" || data.statuscode == "TXN") {
                                        notify("Recharge " + data.status + "! " + data
                                            ?.description, 'success');
                                    } else if (data.status == "pending" || data.statuscode ==
                                        "TUP") {
                                        notify("Recharge " + data.status + "! " + data
                                            ?.description, 'warning');
                                    } else {
                            ...
                            error: function(errors) {
                                notify(errors.responseJSON.status || errors.responseJSON ||
                                    errors.responseJSON.message || "Something went worng",
                                    'error');
                                form.find('button[type="submit"]').html('Pay Now').attr(
                                    'disabled', false).removeClass('btn-secondary');
                            ...
                            // </li>`;
                                    ...
                                    @if ($type == 'mobile')
                                        plandata +=
                                            ...
                                            value.amount + `</button></td><td>` + value.shortDesc +
                                            `</td><td>` + value.longDesc + `</td>
                                    </tr>`;
                                    @else
                                        // var rss = '';
                                        // var validitys = '';
                                        ...
                                        // });
                                        plandata +=
                                            `<tr><td><button class="btn  btn-primary" onclick="setAmount('` +
                                            value.cost +
                                            `')" style="width: 70px;padding:2px 0px;font-size: 15px;"><i class="fa fa-inr"></i> ` +
                                            value.cost + `</button></td><td>` + value?.validity_human +
                                            `</td><td>` + value?.plan_name + `</td>
                                    </tr>`;
                                    @endif
                                    // });
